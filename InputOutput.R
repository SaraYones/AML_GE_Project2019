...
writeOutput=function(path,date,folder1,folder2,features,clusteredRulesMCFS,recalculatedResultRosettalocal,rules,enrichment,enrichmentTitle,flagEnrichment,flag)
{
 ...
 # dir.create(paste(path,date,sep=""))
  ...
  write.csv(recalculatedResultRosettalocal,paste(temp,"/RulesAllGenes-",folder2,Sys.Date(),".csv",sep=""))
  print("Hello")
  saveLineByLine(rules,  paste(temp,"/NetworksAllGenes-",folder2,Sys.Date(),".txt",sep=""))
  print("Hi")
  graphics.off()
  my.plots=vector(1, mode='list');
  #svg(paste(temp,"/HeatMapAllGenes-",Sys.Date(),".svg",sep=""))
  pdf(paste(temp,"/HeatMapAllGenes-",folder2,Sys.Date(),".pdf",sep=""), onefile=TRUE)
  View(clusteredRulesMCFS)
   clusters=heatmap.F(t(clusteredRulesMCFS), colors=c('white','blue'),distmethod='pearson')
   print("Iam here")
   print(flag)
  write.csv(clusters,paste(temp,"/Clusters-",folder2,Sys.Date(),".csv",sep = ""))
 # my.plots[[1]]=recordPlot()
  #savePDF(my.plots,paste("HeatMapAllGenes",Sys.Date()),paste(temp,"/",sep=""))
  
  #Write the Top Annotated Genes
  fileConn<-file(paste(temp,"/TopGenesAnnotated-",folder2,Sys.Date(),".txt",sep = ""))
  ...
  close(fileConn)
     ...
     for (i in 1:length(enrichment))
    {
      ...
      plotEnrichment(enrichment[[i]],paste(temp,"/GOenrichment-",Sys.Date(),".pdf",sep=""))
       Sys.sleep(60)
    }
...
#   plot("Afer Enrichment")
  ...
  write.csv(ordered_metadata_for_exploratory,paste(temp,"/MergedMetaData-",folder2,Sys.Date(),".csv",sep=""))
